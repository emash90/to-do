...
const createTask = async(req, res) => { 
    const { name, author, isComplete, uid, date  } = req.body
...
const getTasks = async(req, res) => {
    try {
        console.log(`filter: ${req.user.id}`);
        const allTasks = await Task.find({ user_id: req.user.id }).sort({createdAt: -1})
...
module.exports = { createTask, getTasks, deleteTask, updateTask, toggleIsComplete }